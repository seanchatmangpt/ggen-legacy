...
The Rust runtime is `ALIVE`, not `BLOCKED:TOOLCHAIN_UNAVAILABLE`. As of 2026-08-03:
...
- `lsp-max` is pinned to `c1cab89bf54fcc9100b3ce75580b3cc3aa8eb852` on branch `fix/wasm4pm-lsp-example-crates-io-dep`, **merged upstream to `master` via PR #22 on 2026-08-04** (re-verified 2026-08-21: `gh pr view 22 --repo seanchatmangpt/lsp-max` reports `state: MERGED`). The originally-admitted rev `220d3251e959f6a58ce0311e995b31a85f98240c` has three independent local-machine-path leaks (an example's dependency escaping the repo via `../../../wasm4pm/...`, a workspace `[patch.crates-io]` table pointing at sibling directories, and an orphaned unreferenced duplicate example with its own broken path) that make cargo unable to enumerate `lsp-max` as a package for any external consumer at all -- this is why `cargo check` previously failed with `no matching package named 'lsp-max' found` regardless of toolchain. PR #22's review and merge happened upstream in the sibling repo, not self-authorized by this repo, consistent with this file's own "Generated contract ownership" and "No self-certification" invariants; no released tag exists yet (`gh api repos/seanchatmangpt/lsp-max/tags`/`.../releases` both empty as of 2026-08-21), so this pin remains a commit-rev pin, provisional only in that narrower sense.
...
- A real stdio replay (`evidence/lsp-contract/stdio-replay-2026-08-03.json`) executed the full `initialize -> initialized -> didOpen -> didChange -> prepareTypeHierarchy -> didClose -> shutdown -> exit` sequence twice against the built binary. Diagnostics, capability advertisement, hierarchy responses, and process exit code (`0`, lawful) are correct and byte-identical across both runs.
- Two findings from the first replay pass are now resolved rather than left open. (1) The process appeared to hang (SIGKILL after 5s): root-caused to two layered causes, both now fixed -- the *replay harness* wasn't answering `lsp-max`'s `client/registerCapability` request (sent during `initialized` for dynamic type-hierarchy registration), which is a real LSP client obligation, not a server defect; and, independently, `src/main.rs` was `?`-propagating `lsp-max`'s `Err(ExitedError(0))` -- its signal for a *lawful* shutdown -- as a generic failure, which `#[tokio::main]`'s default `Termination` impl always turns into exit code `1`. Fixed: `src/main.rs` now destructures `ExitedError` and calls `std::process::exit(code)` directly. (2) The `LSPMAX-ANDON-PUSH-MISSING` diagnostic that fires alongside `GGEN-MANIFEST-001` on open is explained, not a receiver bug: `lsp-max`'s own `Client::publish_diagnostics` (`src/service/client/lsp_methods.rs`) unconditionally flags any ERROR-severity diagnostic whose `source` isn't literally `"lsp-max-andon"` -- `ggen-legacy`'s diagnostics correctly set `source: "ggen-lsp"`; the framework's own code comments show this Andon-push registration API isn't finished upstream. Tracked as an upstream `lsp-max` gap, out of this ticket's authored boundary.
