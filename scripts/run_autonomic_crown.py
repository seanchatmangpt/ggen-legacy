...
ALLOWED_PREFIXES=('autonomic/','fixtures/autonomic/','scripts/autonomic_','scripts/run_autonomic_crown.py','tickets/GL-AUTO-001.md','evidence/autonomic/')
...
def git_guard(base:str)->dict:
 ...
 forbidden=[x for x in files if not x.startswith(ALLOWED_PREFIXES)]
...
def main()->int:
 ...
 steps=[]
 steps.append(command([sys.executable,'-m','py_compile',str(FOUNDRY),str(VERIFIER),str(Path(__file__).resolve())]))
 with tempfile.TemporaryDirectory(prefix='ggen-auto-crown-') as t:
  ...
  gaps=json.loads((out/'GAPS.json').read_text())['gaps']
