...
const EXPECTED: [ExpectedBoundaryTest; EXPECTED_TEST_COUNT] = [
    // ggen_verifier_bad_root_fails_closed
    ExpectedBoundaryTest {
        fn_signature: "fn ggen_verifier_bad_root_fails_closed()",
        exit_code_field: "exit_code: 2,",
        stdout_field: None,
        stderr_field: Some("stderr_needle: Some(\"repository root not found; pass --root <path>\"),"),
        axiom_doc: "/// Covers: an explicit --root with no AGENTS.md anywhere up its ancestry still fails closed with repository root not found",
    },
    // ggen_verifier_good_root_gets_past_root_resolution
    ExpectedBoundaryTest {
        fn_signature: "fn ggen_verifier_good_root_gets_past_root_resolution()",
        exit_code_field: "exit_code: 2,",
        stdout_field: None,
        stderr_field: Some("stderr_needle: Some(\"v26.8.1 verifier refused: No such file or directory\"),"),
        axiom_doc: "/// Covers: with the resolve_root fix, an explicit correct --root gets past root resolution and fails later on a SEPARATE, still-unfixed bug: observe_workspace's unwrapped read of root/Cargo.toml, which does not exist at ggen-legacy's repo root",
    },
